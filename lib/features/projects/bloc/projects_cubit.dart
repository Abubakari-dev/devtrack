...
class ProjectsCubit extends Cubit<ProjectsState> {
  ...
  void init() {
    ...
    _subscription = _repository.getProjectsStream().listen(
      ...
      onError: (e) {
        emit(state.copyWith(status: ProjectsStatus.failure, errorMessage: e.toString()));
      },
    );
  }
  ...
  void _updateList({List<Project>? projects, String? filter, String? searchQuery}) {
    ...
    if (currentFilter != 'All') {
      filtered = filtered.where((p) => p.status.name.toLowerCase() == currentFilter.toLowerCase()).toList();
    }
  ...
  Future<void> deleteProject(String id) async {
    try {
      ...
      emit(state.copyWith(errorMessage: 'Failed to delete project'));
    }
  }
